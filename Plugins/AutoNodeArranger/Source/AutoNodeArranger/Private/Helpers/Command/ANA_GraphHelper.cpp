...
bool ANA_GraphHelper::GetValidGraph(
	SGraphPanel*& graphPanel, FGraphConfig& graphConfig, EANA_GraphType& graphType, bool& isForbiddenGraph)
{
	...
	if (!shrGraphPanel.IsValid()) return false;
...
TSharedPtr<SGraphPanel> ANA_GraphHelper::GetCurrentGraphPanel()
{
	...
	if (!activeTab.IsValid()) return nullptr;
...
{
	if (sWidget->GetTypeAsString() == "SGraphPanel") return TSharedPtr<SGraphPanel>(StaticCastSharedRef<SGraphPanel>(sWidget));
