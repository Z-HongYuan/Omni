...
struct ExtendedComment : OldCppComment, IExtendedComment
{
	using OldCppComment::OldCppComment;
...
private:
	...
	Box commentBox = Box::PosSize(Vector2(), Vector2());
...
public:
	...
	const std::set<ExtendedNode*>& getInsideNodeSet() const override { return insideNodeSet; }

	const std::set<ExtendedNode*>& getTempInsideNodeSet() const override { return tempInsideNodeSet; }
	...
	double getMinPosX() const override { return minPosX; }

	const int64_t& placeOrder() const override { return _placeOrder; }

	int64_t& placeOrder() override { return _placeOrder; }

	void increasePlacedNodeCount() override { placedNodeCount++; }

	bool isFullyPlaced() const override { return placedNodeCount == insideNodeSet.size(); }

	void decreasePlacedNodeCount() override { placedNodeCount--; }

	bool isNonePlaced() const override { return placedNodeCount == 0; }

	void updateCommentBox(const Vector2& commentSpacing) override
	{
		if (insideNodeList.empty() && insideCommentList.empty())
		{
			commentBox = Box::PosSize(pos, size);
			return;
		}
		...
		for (auto insideNode : insideNodeList) commentBox += insideNode->computeNodeBox();
		for (auto insideComment : insideCommentList)
		{
			if (insideComment->getCommentBox().getSize() == Vector2()) insideComment->updateCommentBox(commentSpacing);
			commentBox += insideComment->getCommentBox();
		}
		commentBox = commentBox.extendBox(commentSpacing);
	}

	const Box& getCommentBox() const override { return commentBox; }

	void ApplyFullyPlaced(Vector2 spacing,
		Vector2 commentSpacing,
		bool isAI_Graph,
		std::vector<ExtendedNode*>& placedNodeList,
		std::vector<IExtendedComment*>& placedCommentList) override
	{
		...
		{
			if (placedComment == this || contains(insideCommentList, placedComment)) continue;
		...
		{
			if (contains(insideNodeList, placedNode)) continue;
		...
		if (toAdd.y != 0.)
		{
			for (auto insideNode : insideNodeSet) insideNode->SET_POS(PLACE_Y_COMMENT_NO_OVERLAP, insideNode->pos + toAdd);
			commentBox = commentBox.offsetBox(toAdd);
			for (auto insideComment : insideCommentList) insideComment->commentBox = insideComment->commentBox.offsetBox(toAdd);
		}

		for (auto afterNode : afterNodeList) afterNode->UndoPlaceInY(placedNodeList, placedCommentList);
		for (ExtendedNode* afterNode : afterNodeList) afterNode->PublicPlaceNodeInY(placedNodeList, placedCommentList);
	}
...
private:
	void UpdateDirectionNodeSet(bool isAI_graph)
	{
		...
		for (auto leftNode : leftNodeSet) updateAccessibleExecNodeSet(leftNode, leftNodeSet); // exec on left
		for (auto rightNode : rightNodeSet) updateAccessibleNodeSet(rightNode, rightNodeSet); // all accessible on right
		for (auto leftNode : leftNodeSet) updateAccessibleNodeSet(leftNode, leftNodeSet);	  // remaining on left

		if (isAI_graph) // force inclusion of temp inside nodes for AI graph
		{
			for (auto tempInsideNode : tempInsideNodeSet) addNodeInside(tempInsideNode);
	...
	{
		if (contains(leftNodeSet, currentNode)) return;
		leftNodeSet.insert(currentNode);
		for (auto leftConnectionList : currentNode->leftConnectionListArray)
			for (auto leftConnection : *leftConnectionList) UpdateLeftNodeSet(leftConnection.connectedToNode);
	}

	void UpdateRightNodeSet(ExtendedNode* currentNode)
	{
		if (contains(rightNodeSet, currentNode)) return;
		rightNodeSet.insert(currentNode);
		for (auto rightConnectionList : currentNode->rightConnectionListArray)
			for (auto rightConnection : *rightConnectionList) UpdateRightNodeSet(rightConnection.connectedToNode);
	}
	...
	void updateAccessibleExecNodeSet(ExtendedNode* currentNode, std::set<ExtendedNode*>& accessibleNodeSet)
	{
		...
		for (auto execConnectionList : currentNode->execConnectionListArray)
		{
			for (auto execConnection : *execConnectionList)
			{
				...
				if (contains(rightNodeSet, nextNode) || contains(leftNodeSet, nextNode) || contains(insideNodeSet, nextNode)
					|| contains(tempInsideNodeSet, nextNode))
					continue;
	...
	void GetPosForNoOverlapCommentInY(Vector2& toAdd,
		Vector2 spacing,
		Vector2 commentSpacing,
		...
		while (hasIntersection)
		{
			...
			for (auto placedComment : placedCommentList)
			{
				...
				const Box placedCommentBox
					= placedComment->extendBoxWithComment(placedComment->getCommentBox(), this, commentSpacing)
						  .extendBox(halfSpacingY)
						  .offsetBox(halfSpacingY);
				if (commBox.intersectWith(placedCommentBox))
				{
					hasIntersection = true;
					toAdd.y += placedCommentBox.Max.y - commBox.Min.y + 1;
				}
			}
			for (auto placedNode : placedNodeList)
			{
				if (contains(insideNodeSet, placedNode) || getOrComputeCommentDiffCount(placedNode) > 1) continue;
				const Box commBox
					= extendBoxWithComment(box, placedNode, commentSpacing).offsetBox(toAdd).extendBox(-COMMENT_HEADER);
				...
				{
					hasIntersection = true;
					toAdd.y += placedNodeBox.Max.y - commBox.Min.y + 1;
				}
			}
		}
	}
