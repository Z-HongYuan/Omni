...
#include "../../CppGraphArranger/DataTypes/Instruction/LinkInstruction.h"
#include "../../CppGraphArranger/DataTypes/Instruction/NodeInstruction.h"
...
struct ExtendedNode : OldCppNode, ACommentAndHeaderDiffCounter
{
	using OldCppNode::OldCppNode;
	...
	struct Connection
	{
		...
		Connection(bool _bLeft,
			bool _bExec,
			bool _bCompact,
			ExtendedPin* _connectedFromPin,
			ExtendedPin* _connectedToPin,
			ExtendedNode* _connectedFromNode,
			ExtendedNode* _connectedToNode,
			Vector2 _deltaPos) :
			bLeft(_bLeft), bExec(_bExec), bCompact(_bCompact), connectedFromPin(_connectedFromPin),
			connectedToPin(_connectedToPin), connectedFromNode(_connectedFromNode), connectedToNode(_connectedToNode),
			deltaPos(_deltaPos)
		{
		}
		...
		{
			auto toPinIt = find(
				connectedFromPin->connectedToPinIDList.begin(), connectedFromPin->connectedToPinIDList.end(), connectedToPin->ID);
			if (toPinIt != connectedFromPin->connectedToPinIDList.end()) connectedFromPin->connectedToPinIDList.erase(toPinIt);
			auto fromPinIt = find(
				connectedToPin->connectedToPinIDList.begin(), connectedToPin->connectedToPinIDList.end(), connectedFromPin->ID);
			if (fromPinIt != connectedToPin->connectedToPinIDList.end()) connectedToPin->connectedToPinIDList.erase(fromPinIt);

			auto& connectedToExtendedConnectionList = connectedToNode->touchExtendedConnectionList(!bLeft, bExec);
			auto oppositeConnectionIt = GetOppositeConnection();
			if (oppositeConnectionIt != connectedToExtendedConnectionList.end())
				connectedToExtendedConnectionList.erase(oppositeConnectionIt);

			auto& connectedFromExtendedConnectionList = connectedFromNode->touchExtendedConnectionList(bLeft, bExec);
			auto extendedConnectionIt
				= std::find(connectedFromExtendedConnectionList.begin(), connectedFromExtendedConnectionList.end(), *this);
			if (extendedConnectionIt != connectedFromExtendedConnectionList.end())
				connectedFromExtendedConnectionList.erase(extendedConnectionIt);
		...
		std::vector<Connection>::iterator GetOppositeConnection()
		{
			...
			return std::find(
				connectedToExtendedConnectionList.begin(), connectedToExtendedConnectionList.end(), connectionToFindOpposite);
		}
		...
		Box ComputeConnectionBox(bool& isConnectionAligned) const
		{
			...
			return Box::PosSize(Vector2(leftPinTargetPos.x, leftPinTargetPos.y - 0.5 * graphConfig->lineWidth),
	...
	private:
		...
		{
			return Connection(
				!bLeft, bExec, bCompact, connectedToPin, connectedFromPin, connectedToNode, connectedFromNode, Vector2());
		}
	};
	// ======== ATTRIBUTES ========
...
private:
	...
	{
		PlaceInYParam() {}
		PlaceInYParam(const std::vector<ExtendedNode*>& _placedNodeList,
			...
			placedNodeList(_placedNodeList), placedCommentList(_placedCommentList), placedConnectionSet(_placedConnectionSet),
...
private:
	...
	static inline std::chrono::time_point<std::chrono::steady_clock> timeout;
	static inline int stackCount = 0;
...
public:
	const std::set<const ACommentAndHeaderDiffCounter*> computeContainedByCommentSet() const override
	{
		...
		for (auto containedByIExtendedComment : containedByIExtendedCommentSet) result.insert(containedByIExtendedComment);
		return result;
	}
...
public:
	static void InitializeTimeout() { timeout = std::chrono::steady_clock::now() + std::chrono::seconds(3); }
	...
	static void InitializeStack() { stackCount = 0; }

	static void IncrementStackAndCheck()
	{
		if (++stackCount > 8000) throw std::runtime_error("Stack Overflow");
	}

	static void DecrementStack() { --stackCount; }

	int64_t getPlaceOrder() const { return placeOrder; }

	void updateConnectionListArray()
	{
		allConnectionListArray
			= {{&leftExecConnectionList, &leftNonExecConnectionList, &rightExecConnectionList, &rightNonExecConnectionList}};
	...
	void computeNodeExtensionForPin(std::vector<ExtendedPin>& pinList)
	{
		for (auto pinIDList : pinIDListArray)
			for (auto& pinID : *pinIDList) pinList[pinID].computePinExtension(ID);
		...
		for (int i = 0; i < pinIDListArray.size(); ++i)
		{
			for (auto& connectedFromPinID : *pinIDListArray[i])
			{
				...
				for (auto connectedToPinID : connectedFromPin.connectedToPinIDList)
				{
					...
					bool isCompacted
						= graphConfig->bCompact && graphConfig->GetIsBlueprintGraph() && !isExec
					...
					allConnectionListArray[i]->emplace_back(isLeft,
						isExec,
						isCompacted,
						&connectedFromPin,
						&connectedToPin,
						...
						isCompacted
							? Vector2(0.,
								  isLeft ? (size.y + graphConfig->spacing.y + commentDiffCount * graphConfig->commentSpacing.y)
										 : (-(connectedToNode.size.y + graphConfig->spacing.y
											   + commentDiffCount * graphConfig->commentSpacing.y)))
						: graphConfig->GetIsAI_Graph()
							? Vector2(0.5 * (connectedToNode.size.x - size.x) * (isLeft ? 1. : -1.),
								  isLeft ? (-(connectedToNode.size.y + graphConfig->spacing.y
											   + commentDiffCount * graphConfig->commentSpacing.y))
										 : (size.y + graphConfig->spacing.y + commentDiffCount * graphConfig->commentSpacing.y))
							: Vector2((((graphConfig->GetIsMaterialOrSoundGraph() != isLeft) ? -1. : 1.)
	...
	void computeCompactNodeExtension()
	{
		for (int i = 0; i < allConnectionListArray.size(); ++i)
		{
			...
			{
				if (connection.bCompact) continue;
				...
				connection.deltaPos
					= Vector2((isLeft ? -1. : 1.) * (maxSizeX + spacingX + commentDiffCount * graphConfig->commentSpacing.x),
	...
	{
		if (contains(connectedNodeSet, this)) return;
		...
		if (leftExecConnectionList.empty())
		{
			...
			*/
			bool isBetterToChange = globalRootNode == nullptr ? true
									: globalRootNode->rightExecConnectionList.empty() != rightExecConnectionList.empty()
										? globalRootNode->rightExecConnectionList.empty()
									: globalRootNode->pos.y != pos.y
										? globalRootNode->pos.y > pos.y
										: (graphConfig->GetIsMaterialOrSoundGraph() != globalRootNode->pos.x > pos.x);
			if (isBetterToChange) globalRootNode = this;
		}
...
private:
	...
	{
		return leftNonExecConnectionList.empty() || !leftNonExecConnectionList[0].bCompact
				   ? nullptr
				   : leftNonExecConnectionList[0].connectedToNode;
	}

	bool GetIsExecConnectedAtLeft()
	{
		if (GetIsVisited()) return true;
		for (auto& leftExecConnection : leftExecConnectionList)
		{
			auto leftNode = leftExecConnection.connectedToNode;
			if (leftNode->rerouteLoopNodeParent) continue;
			if (leftNode->GetIsExecConnectedAtLeft()) return true;
		}
		return false;
	}
	...
	bool GetWillBePlacedAtRight()
	{
		...
		if (GetIsExecConnectedAtLeft()) return true;
		for (auto rightNonExecConnection : rightNonExecConnectionList)
		{
			if (rightNonExecConnection.connectedToNode->GetWillBePlacedAtRight()) return true;
		}
		return false;
	}
...
public:
	void SetPos(const Vector2& toPos, const char* stepName, std::string filename, int line, const char* functionName)
	{
		...
		HIGHLIGHT(stepName, nodeInstructionList->back(), ID);
		pos = toPos;
	}

	Box computeNodeBox() const { return Box::PosSize(pos, size); }

	std::vector<Connection>& touchExtendedConnectionList(bool bLeft, bool bExec)
	{
		return (bLeft	? bExec ? leftExecConnectionList : leftNonExecConnectionList
				: bExec ? rightExecConnectionList
						: rightNonExecConnectionList);
	}
...
private:
	std::set<Connection*> ComputeConnectionSetToAppend(const std::vector<ExtendedNode*>& placedNodeList) const
	{
		...
		for (auto allConnectionList : allConnectionListArray)
		{
			...
			{
				if (contains(placedNodeList, connection.connectedToNode)) result.insert(&connection);
			}
		}
		return result;
	}
...
public:
	...
	{
		if (!GetIsVisited() || contains(onRightExecVisitedNodeSet, this)) return;
	...
	void UpdatePosForNoOverlapNodeInY(
	...
	void UndoPlaceInY(std::vector<ExtendedNode*>& placedNodeList, std::vector<IExtendedComment*>& placedCommentList)
	{
		...
		for (auto containedByIExtendedComment : containedByIExtendedCommentSet)
		{
			if (containedByIExtendedComment->isFullyPlaced())
				placedCommentList.erase(
					std::find(placedCommentList.begin(), placedCommentList.end(), containedByIExtendedComment));
			containedByIExtendedComment->decreasePlacedNodeCount();
			if (containedByIExtendedComment->isNonePlaced()) containedByIExtendedComment->placeOrder() = -1;
		}
	}
	...
	void PublicPlaceNodeInY(std::vector<ExtendedNode*>& placedNodeList, std::vector<IExtendedComment*>& placedCommentList)
	{
		...
		if (refNode)
		{
			if (refExec) refNode->PlaceExecAtRightInY(tmpPlaceInYParam);
			else
...
private:
	...
	void PlaceAtLeftInX(std::queue<ExtendedNode*>& toPlaceLaterNodeQueue)
	{
		...
		for (auto visitedNode : visitedNodeSet) toPlaceLaterNodeQueue.push(visitedNode);
	}
	...
	{
		for (auto notFreeNodeAtLeft : notFreeNodeAtLeftList) notFreeNodeAtLeft->SetIsNotVisited();
		for (auto notFreeNodeAtLeft : notFreeNodeAtLeftList) notFreeNodeAtLeft->PlaceOnLeftInX(notFreeNodeAtLeftList);
	}
	...
	void PlacePlaceableOnRightInX(const std::set<ExtendedNode*>& visitedNodeSet)
	{
		...
		for (auto visitedNode : visitedNodeSet) visitedNode->PlaceOnRightInX(visitedNodeList);
		std::vector<ExtendedNode*> nodeToPlaceOnRightList;
		for (auto visitedNode : visitedNodeSet)
		{
			if (!graphConfig->GetIsCompactBlueprintGraph() && !visitedNode->isToPlaceOnRight) continue;
			visitedNode->SetIsNotVisited();
			nodeToPlaceOnRightList.push_back(visitedNode);
		}
		for (auto nodeToPlaceOnRight : nodeToPlaceOnRightList) nodeToPlaceOnRight->PlaceOnRightInX(nodeToPlaceOnRightList);
	}

	void PlaceOnLeftInX(const std::vector<ExtendedNode*>& notFreeNodeAtLeftList)
	{
		if (GetIsVisited() || !contains(notFreeNodeAtLeftList, this)) return;
		SetIsVisited();
		...
		for (auto leftConnectionList : leftConnectionListArray)
		{
			...
			{
				if (!leftConnection.connectedToNode->GetIsVisited()
					&& !contains(notFreeNodeAtLeftList, leftConnection.connectedToNode))
					continue;
				...
				if (posX < newPosX != graphConfig->GetIsMaterialOrSoundGraph()) posX = newPosX;
				...
				if (leftConnection.bExec)
				{
					...
					for (auto containedByIExtendedComment : containedByIExtendedCommentSet)
					{
						...
						double newMinPosX = containedByIExtendedComment->updateMinPosX(
												newPosX + sizeOffset, graphConfig->GetIsMaterialOrSoundGraph())
											- sizeOffset;
						if (posX < newMinPosX != graphConfig->GetIsMaterialOrSoundGraph()) posX = newMinPosX;
					}
				}
			}
		}
	...
	{
		if (GetIsVisited() || !contains(nodeAtLeftList, this)) return;
		SetIsVisited();
		double scaleToAdd = graphConfig->GetIsMaterialOrSoundGraph() ? -1. : 1.;
		double posX = MAX_DOUBLE * scaleToAdd;
		...
		for (auto rightConnectionList : rightConnectionListArray)
		{
			for (auto rightConnection : *rightConnectionList)
			{
				if (!rightConnection.connectedToNode->GetIsVisited()
					&& !contains(nodeAtLeftList, rightConnection.connectedToNode))
					continue;
	...
	bool GetNotFreeNodeAtLeftList(std::vector<ExtendedNode*>& notFreeNodeOnLeftList,
		bool isOnLeftOfRerouteOneOutput,
		std::set<ExtendedNode*>& visitedNodeSet)
	{
		isToPlaceOnRight &= !isOnLeftOfRerouteOneOutput;
		if (GetIsVisited()) return true;
		if (contains(visitedNodeSet, this)) return contains(notFreeNodeOnLeftList, this);
		visitedNodeSet.insert(this);
		bool isNotFree = false;
		for (auto leftConnectionList : leftConnectionListArray)
			for (auto leftConnection : *leftConnectionList)
				if (leftConnection.connectedToNode->GetNotFreeNodeAtLeftList(
						notFreeNodeOnLeftList, isOnLeftOfRerouteOneOutput || this->isRerouteNodeOneOutput, visitedNodeSet))
					isNotFree = true;
		if (isNotFree) notFreeNodeOnLeftList.push_back(this);
		else
	...
	{
		if (!PlaceNodeInY(minPosY, placeInYParam)) return;
		IncrementStackAndCheck();
	...
	bool PlaceNodeInY(double minPosY, PlaceInYParam& placeInYParam)
	{
		...
		if (graphConfig->bCenterize)
		{
			...
			if (remainingCenterizeNodeToPlaceCount == 0) centerizeBP(placeInYParam);
		}
		...
		if (graphConfig->bCenterize)
		{
			for (auto forCenterizeToUpdateNode : forCenterizeToUpdateNodeList)
			{
				...
				if (forCenterizeToUpdateNode->remainingCenterizeNodeToPlaceCount == 0)
				{
					...
					forCenterizeToUpdateNode->UpdatePosForNoOverlapNodeInY(
	...
	void PlaceNonExecAtLeftInY(PlaceInYParam& placeInYParam)
	{
		...
		if (!leftNonExecConnectionList.empty())
		{
			...
			leftNonExecConnectionList[0].connectedToNode->GetFirstAtDirectionMinPosY(
				newMinPosY, true, false, placeInYParam.placedNodeList, placeInYParam.placedCommentList);
	...
	void PlaceExecAtRightInY(PlaceInYParam& placeInYParam)
	{
		...
		for (auto rightExecConnection : rightExecConnectionList)
		{
			...
			rightExecConnection.connectedToNode->GetFirstAtDirectionMinPosY(
				minPosY, false, true, placeInYParam.placedNodeList, placeInYParam.placedCommentList);
		...
		if (rerouteLoopNodeChild) rerouteLoopNodeChild->PlaceRerouteLoopNodeInXY(placeInYParam);
	}
	...
	void PlaceNonExecAtRightInY(PlaceInYParam& placeInYParam)
	{
		...
		if (!rightNonExecConnectionList.empty())
		{
			...
			for (auto rightNonExecConnection : rightNonExecConnectionList)
			{
				if (rightNonExecConnection.connectedToNode->GetWillBePlacedAtRight()) continue;
	...
	void UpdatePlaceOrder()
	{
		...
		for (auto containedByIExtendedComment : containedByIExtendedCommentSet)
		{
			if (containedByIExtendedComment->placeOrder() == -1) containedByIExtendedComment->placeOrder() = getNewPlaceOrder();
		}
	}
	...
	void ApplyFullyPlacedCommentOverlap(PlaceInYParam& placeInYParam, bool bIncreasePlacedNodeCount = true)
	{
		for (auto containedByIExtendedComment : containedByIExtendedCommentSet)
		{
			if (bIncreasePlacedNodeCount) containedByIExtendedComment->increasePlacedNodeCount();
			if (containedByIExtendedComment->isFullyPlaced())
			{
				...
				containedByIExtendedComment->ApplyFullyPlaced(graphConfig->spacing,
					graphConfig->commentSpacing,
					graphConfig->GetIsAI_Graph(),
					placeInYParam.placedNodeList,
	...
	void PlaceExecOnlyMostAtLeftInY(double minPosY, PlaceInYParam& placeInYParam)
	{
		...
		else
		{
			...
			for (auto leftExecConnection : leftExecConnectionList)
				leftExecConnection.connectedToNode->PlaceExecOnlyMostAtLeftInY(
					minPosY + leftExecConnection.deltaPos.y, placeInYParam);
		}
	}

	void GetFirstAtDirectionMinPosY(double& newMinPosY,
		bool bLeft,
		bool bExec,
		const std::vector<ExtendedNode*>& placedNodeList,
		const std::vector<IExtendedComment*>& placedCommentList)
	{
		...
		while (i < connectionList.size() && (connectionList[i].bCompact || connectionList[i].connectedToNode->GetIsVisited()))
			i++;
		if (i == connectionList.size()) return;
		newMinPosY += connectionList[i].deltaPos.y;
		connectionList[i].connectedToNode->GetFirstAtDirectionMinPosY(
			newMinPosY, bLeft, bExec, placedNodeList, placedCommentList);
		newMinPosY -= connectionList[i].deltaPos.y;
	}

	void GetPosForNoOverlapNodeInY(double& minPosY,
		const std::vector<ExtendedNode*>& placedNodeList,
		const std::vector<IExtendedComment*>& placedCommentList)
	{
		...
		while (hasIntersection)
		{
			...
			for (auto placedComment : placedCommentList)
			{
				if (contains(placedComment->getInsideNodeSet(), this) || getOrComputeCommentDiffCount(placedComment) != 1)
					continue;
				const Box nodeBox = extendBoxWithComment(box, placedComment, graphConfig->commentSpacing)
										.offsetBox(Vector2(0., minPosY - pos.y));
				const Box placedCommentBox
					= placedComment->extendBoxWithComment(placedComment->getCommentBox(), this, graphConfig->commentSpacing)
						  .extendBox(halfSpacingY)
						  .offsetBox(halfSpacingY);
				if (nodeBox.intersectWith(placedCommentBox))
				{
					hasIntersection = true;
					minPosY += placedCommentBox.Max.y - nodeBox.Min.y + 1;
				}
			}
			for (auto placedNode : placedNodeList)
			{
				if (placedNode == this || getOrComputeCommentDiffCount(placedNode) != 0) continue;
				const Box nodeBox
					= extendBoxWithComment(box, placedNode, graphConfig->commentSpacing).offsetBox(Vector2(0., minPosY - pos.y));
				const Box placedNodeBox
					= placedNode->extendBoxWithComment(placedNode->computeNodeBox(), this, graphConfig->commentSpacing)
						  .extendBox(halfSpacingY)
						  .offsetBox(halfSpacingY);
				if (nodeBox.intersectWith(placedNodeBox))
				{
					hasIntersection = true;
					minPosY += placedNodeBox.Max.y - nodeBox.Min.y + 1;
				}
			}
		}
	}

	void PlaceForNoConnectionOverlapInY(
		const std::set<Connection*>& placedConnectionSet, const std::set<Connection*>& connectionSetToAppend)
	{
		...
		for (auto connection : connectionSetToAppend)
		{
			...
			if (!isConnectionAligned) continue;
			for (auto placedConnection : placedConnectionSet)
			{
				if (*connection == *placedConnection || *connection == *placedConnection->GetOppositeConnection()) continue;
	...
	{
		if (GetIsVisited() || !rerouteLoopNodeParent->GetIsVisited()
			|| !rerouteLoopNodeBrother->rerouteLoopNodeParent->GetIsVisited())
			return;
		...
		SET_POS(PLACE_Y_REROUTE_LOOP,
			rerouteLoopNodeParent->pos
				- (leftExecConnectionList.empty() ? leftNonExecConnectionList : leftExecConnectionList)[0].deltaPos);
		rerouteLoopNodeBrother->SET_POS(PLACE_Y_REROUTE_LOOP,
			rerouteLoopNodeBrother->rerouteLoopNodeParent->pos
				+ (rightExecConnectionList.empty() ? rightNonExecConnectionList : rightExecConnectionList)[0].deltaPos);
		...
		while (minPosY != oldMinPosY)
		{
			...
			GetPosForNoOverlapNodeInY(minPosY, placeInYParam.placedNodeList, placeInYParam.placedCommentList);
			rerouteLoopNodeBrother->GetPosForNoOverlapNodeInY(
				minPosY, placeInYParam.placedNodeList, placeInYParam.placedCommentList);
		}
		SET_POS(PLACE_Y_NO_NODE_OVERLAP, Vector2(pos.x, minPosY));
	...
	void offsetAll(const Vector2& toAdd)
	{
		...
		for (auto allConnectionList : allConnectionListArray)
			for (auto& connection : *allConnectionList) connection.connectedToNode->offsetAll(toAdd);
	}
	...
	void AI_PlaceInX() { AI_PlaceNodeInX(pos.x); }
	...
	std::vector<ExtendedNode*> AI_PlaceNodeInX(double minPosX)
	{
				...
				for (auto justPlacedNode : justPlacedNodeList)
				{
					newMinPosX = std::max(newMinPosX,
						justPlacedNode->pos.x + justPlacedNode->size.x + graphConfig->spacing.x
							+ justPlacedNode->getOrComputeCommentDiffCount(rightExecConnection.connectedToNode)
								  * graphConfig->commentSpacing.x);
				}
		...
		if (!rightExecConnectionList.empty())
		{
			...
			if (graphConfig->bCenterize)
			{
				Vector2 toAdd = Vector2((rightExecConnectionList.back().connectedToNode->computeNodeBox().Max.x
											- rightExecConnectionList.front().connectedToNode->computeNodeBox().Min.x - size.x)
											* -0.5,
					0);
	...
	{
		if (contains(visitedNodeSet, this)) return;
		visitedNodeSet.insert(this);
		SET_POS(AI_PLACE_OFFSET, pos + toAdd);
		for (auto rightExecConnection : rightExecConnectionList)
	...
	void AI_PlaceInY() { PlaceInY(); }

	// ======== CENTER ARRANGEMENT ========

	void updateRemainingCenterizeNodeToPlaceCount()
	{
		if (rerouteLoopNodeParent || (!graphConfig->GetIsMaterialOrSoundGraph() && !leftExecConnectionList.empty())) return;
	...
	void centerizeBP(PlaceInYParam& placeInYParam)
	{
		...
		for (auto& connection : graphConfig->GetIsMaterialOrSoundGraph() ? rightExecConnectionList : leftNonExecConnectionList)
		{
			if (getOrComputeCommentDiffCount(connection.connectedToNode) != 0) continue;
			sumCoeff++;
...
inline std::vector<NodeInstruction>* ExtendedNode::nodeInstructionList = nullptr;
inline CppGraphConfig* ExtendedNode::graphConfig = nullptr;
inline bool* ExtendedNode::bVisitedValue = nullptr;
