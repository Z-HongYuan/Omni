...
struct OldCppNode
{
	...
	OldCppNode() : OldCppNode(-1, std::string(), Vector2(), Vector2(), false) {}
	OldCppNode(int _ID, std::string _name, Vector2 _pos, Vector2 _size, bool _isRerouteNode) :
	...
	{
		return (bLeft ? bExec ? leftExecPinIDList : leftNonExecPinIDList : bExec ? rightExecPinIDList : rightNonExecPinIDList);
	}

	// ======== READ/WRITE ========

	friend std::istream& operator>>(std::istream& is, OldCppNode& node)
	{
		is >> node.ID;
	...
	friend std::ostream& operator<<(std::ostream& os, const OldCppNode& node)
	{
		os << node.ID << " ";
