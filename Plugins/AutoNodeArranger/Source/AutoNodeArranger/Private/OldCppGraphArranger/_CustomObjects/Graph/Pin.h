...
struct OldCppPin
{
	...
	OldCppPin() : OldCppPin(-1, std::string(), Vector2()) {}
	OldCppPin(int _ID, std::string _name, Vector2 _offset) : ID(_ID), name(_name), offset(_offset)
	{
	...
	friend std::istream& operator>>(std::istream& is, OldCppPin& pin)
	{
		is >> pin.ID;
	...
	friend std::ostream& operator<<(std::ostream& os, const OldCppPin& pin)
	{
		os << pin.ID << " ";
