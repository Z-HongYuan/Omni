...
struct OldConnectedGraph
{
	struct CommentRelativeInfo
	{
		...
		CommentRelativeInfo(const OldConnectedGraph& from, const OldConnectedGraph& to)
		{
...
private:
	Box connectedGraphBox = Box::PosSize(Vector2(), Vector2());
	Box commentConnectedGraphBox = Box::PosSize(Vector2(), Vector2());

	// ======== CONSTRUCTOR ========

public:
	OldConnectedGraph(ExtendedNode* selectedNode, std::vector<ExtendedComment>& commentList)
	{
		...
		for (auto connectedNode : connectedNodeSet)
			for (auto containedByIExtendedComment : connectedNode->containedByIExtendedCommentSet)
				connectedCommentSet.insert(
					&commentList[containedByIExtendedComment->getID()]); // other nodes contained by comment could be considered
	}
	...
	void updateConnectedGraphBox()
	{
		...
		for (auto connectedNode : connectedNodeSet) connectedGraphBox += connectedNode->computeNodeBox();
	}
		...
		for (auto connectedNode : connectedNodeSet)
			commentConnectedGraphBox
				+= connectedNode->computeNodeBox().computeBoxToNotOverlapInX(connectedNode->containedByIExtendedCommentSet.size(),
					connectedNode->containedByIExtendedCommentSet.size(),
					Vector2(),
					Vector2(commentSpacing.x, -commentSpacing.y));
	}

	Vector2 getMinPos(const Vector2& spacing, const Vector2& commentSpacing) const
	{
		return connectedGraphBox.computeBoxToNotOverlapInX(0, connectedCommentSet.size(), spacing, commentSpacing).Min;
	}
