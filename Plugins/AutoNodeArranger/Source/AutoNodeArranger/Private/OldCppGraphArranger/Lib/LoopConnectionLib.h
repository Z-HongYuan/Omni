...
#include "../../CppGraphArranger/DataTypes/Instruction/RerouteInstruction.h"
...
public:
	...
	LoopConnectionLib(const CppGraphConfig* _graphConfig,
		...
		graphConfig(_graphConfig), rerouteInstructionList(_rerouteInstructionList), linkInstructionList(_linkInstructionList),
		connectedToNodeSelectionNodeSet(_connectedToNodeSelectionNodeSet), pinList(_pinList), nodeList(_nodeList)
	{
	}
	...
	void AutoGenerateRerouteLoopNode()
	{
		...
		{
			if (!connection->connectedFromNode->isRerouteNodeOneOutput) GenerateRerouteLoopNode(true, connection);
			if (!connection->connectedToNode->isRerouteNodeOneOutput) GenerateRerouteLoopNode(false, connection);
		}
	}
		...
		for (auto connection : GetConnectionToBreakList(false))
		{
			...
			if (isConnectedFromRerouteNodeOneOutput)
			{
				ExtendedNode::Connection* inputConnection = connection->connectedFromNode->leftExecConnectionList.empty()
																? &connection->connectedFromNode->leftNonExecConnectionList[0]
																: &connection->connectedFromNode->leftExecConnectionList[0];
			...
			if (isConnectedToRerouteNodeOneOutput)
			{
				ExtendedNode::Connection* outputConnection = connection->connectedToNode->rightExecConnectionList.empty()
																 ? &connection->connectedToNode->rightNonExecConnectionList[0]
																 : &connection->connectedToNode->rightExecConnectionList[0];
	...
	void BreakAllLoops()
	{
		...
		else
		{
			for (auto connection : GetConnectionToBreakList(true))
			{
				// add to linkInstructionList in order to let the connection broken
				linkInstructionList->push_back(connection->breakConnection());
				HIGHLIGHT("LoopLinkHandler", linkInstructionList->back());
			}
		}
	}
	...
	std::vector<ExtendedNode::Connection*> GetConnectionToBreakList(bool bNextConnection)
	{
		...
		for (auto connectedToNodeSelectionNode : *connectedToNodeSelectionNodeSet)
			RecursiveGetConnectionToBreakList(
				connectionToBreakList, connectedToNodeSelectionNode, bNextConnection, pathToCurrentNode, pathToCurrentConnection);
		return connectionToBreakList;
	}

	void RecursiveGetConnectionToBreakList(std::vector<ExtendedNode::Connection*>& connectionToBreakList,
		ExtendedNode* currentNode,
		bool bNextConnection,
		std::vector<ExtendedNode*>& pathToCurrentNode,
		std::vector<ExtendedNode::Connection*>& pathToCurrentConnection)
	{
		...
		for (auto connectionList : pathToCurrentNode.back()->rightConnectionListArray)
		{
			...
			{
				if (contains(connectionToBreakList, &connection)) continue;
				...
				if (contains(pathToCurrentNode, connection.connectedToNode))
				{
					...
					if (bNextConnection && pathToCurrentConnection.size() > 2) index++;
					auto connectionToAdd = pathToCurrentConnection[index % pathToCurrentConnection.size()];
					if (!contains(connectionToBreakList, connectionToAdd)) connectionToBreakList.push_back(connectionToAdd);
				}
				else
					RecursiveGetConnectionToBreakList(connectionToBreakList,
						connection.connectedToNode,
						bNextConnection,
						pathToCurrentNode,
						pathToCurrentConnection);
	...
	int GetConnectionToBreakIndex(const std::vector<ExtendedNode::Connection*>& connectionList)
	{
		...
		for (int i = 0; i < connectionList.size(); i++)
		{
			if (connectionList[i]->connectedFromNode->isRerouteNodeOneOutput
				&& connectionList[i]
					   ->connectedToNode->isRerouteNodeOneOutput) // if connection between 2 reroute nodes with one output
				return i;
			float diffPosX_I = connectionList[i]->connectedToNode->pos.x - connectionList[i]->connectedFromNode->pos.x;
			if (connectionList[i]
					->connectedToNode->isRerouteNodeOneOutput) // if connection finish with a reroute node with one output
			{
	...
	void GenerateRerouteLoopNode(bool isLeft, ExtendedNode::Connection*& connection)
	{
		...
		Vector2 pos = isLeft ? Vector2(leftNode->pos.x + leftNode->size.x - rerouteSize.x,
								   leftNode->pos.y + leftNode->size.y + graphConfig->spacing.y)
		...
		HIGHLIGHT("LoopLinkHandler", rerouteInstructionList->back());
		...
		// Make connections
		linkInstructionList->push_back(
			makeLinkTo(true, connection->bExec, false, inputPin, leftPin, rerouteNode, leftNode, Vector2()));
		HIGHLIGHT("LoopLinkHandler", linkInstructionList->back());
		...
		linkInstructionList->push_back(
			makeLinkTo(false, connection->bExec, false, outputPin, rightPin, rerouteNode, rightNode, Vector2()));
		HIGHLIGHT("LoopLinkHandler", linkInstructionList->back());
	...
	static LinkInstruction makeLinkTo(bool bLeft,
		bool bExec,
		bool bCompact,
		ExtendedPin* connectedFromPin,
		ExtendedPin* connectedToPin,
		ExtendedNode* connectedFromNode,
		ExtendedNode* connectedToNode,
		Vector2 deltaPos)
	{
		...
		connectedFromNode->touchExtendedConnectionList(bLeft, bExec)
			.emplace_back(bLeft, bExec, bCompact, connectedFromPin, connectedToPin, connectedFromNode, connectedToNode, deltaPos);
		connectedToNode->touchExtendedConnectionList(!bLeft, bExec)
			.emplace_back(
				!bLeft, bExec, bCompact, connectedToPin, connectedFromPin, connectedToNode, connectedFromNode, deltaPos);
