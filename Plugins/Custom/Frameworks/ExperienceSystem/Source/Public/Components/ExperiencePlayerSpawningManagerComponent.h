...
#include "ExperiencePlayerSpawningManagerComponent.generated.h"

#define UE_API EXPERIENCESYSTEM_API

class AExperiencePlayerStart;

/**
 * 出生点管理组件
 */
UCLASS(MinimalAPI)
class UExperiencePlayerSpawningManagerComponent : public UGameStateComponent
{
	GENERATED_BODY()

public:
	UE_API UExperiencePlayerSpawningManagerComponent(const FObjectInitializer& ObjectInitializer);
...
protected:
	...
	UE_API virtual AActor* OnChoosePlayerStart(AController* Player, TArray<AExperiencePlayerStart*>& PlayerStarts);
	...
	UE_API APlayerStart* GetFirstRandomUnoccupiedPlayerStart(AController* Controller, const TArray<AExperiencePlayerStart*>& FoundStartPoints) const;

private:
	// 以下三个函数由 AExperienceGameMode 转发调用，不要在别处直接调用
	...
	friend class AExperienceGameMode;

	// 缓存场景中所有的 AExperiencePlayerStart
	UPROPERTY(Transient)
	TArray<TWeakObjectPtr<AExperiencePlayerStart>> CachedPlayerStarts;
