...
#include "ExperienceBotCreationComponent.generated.h"
...
class UExperienceBotCreationComponent : public UGameStateComponent
{
	GENERATED_BODY()

public:
	UE_API UExperienceBotCreationComponent(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
...
protected:
	...
	// 使用的 Bot 控制器类
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = Gameplay)
	TSubclassOf<AExperienceAIController> BotControllerClass;
	...
	// 已经创建出来的 Bot 列表
	UPROPERTY(Transient)
	TArray<TObjectPtr<AExperienceAIController>> SpawnedBotList;
	...
	UE_API void K2_OnBotSpawned(AExperienceAIController* BotController);
...
private:
	// 体验加载完成后的回调
	UE_API void OnExperienceLoaded(const UExperienceDefinition* Experience);
